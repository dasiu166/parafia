...
public class EditParishionerDialog extends JDialog implements ActionListener {
	...
	public static void main(String[] args) {
		try {
			EditParishionerDialog dialog = new EditParishionerDialog(null,
					new Parishioner());
	...
	public EditParishionerDialog(JFrame owner) {
		...
		getContentPane().add(scrollContentPanel, BorderLayout.CENTER);
		{
			...
			label_Data_.setFont(new Font("Times New Roman", Font.BOLD
					| Font.ITALIC, 20));
			...
			label_Address_.setFont(new Font("Times New Roman", Font.BOLD
					| Font.ITALIC, 20));
			...
			lblDaty.setFont(new Font("Times New Roman",
			...
			btnDane.setIcon(new ImageIcon(EditParishionerDialog.class.getResource("/icons/sign-up-icon.png")));
			btnDane.addActionListener(this);

			btnAdres = new JButton("Aktualizuj");
			btnAdres.setIcon(new ImageIcon(EditParishionerDialog.class.getResource("/icons/sign-up-icon.png")));
			btnAdres.addActionListener(this);

			btnDaty = new JButton("Aktualizuj");
			btnDaty.setIcon(new ImageIcon(EditParishionerDialog.class.getResource("/icons/sign-up-icon.png")));
				...
				gl_panelContent.createParallelGroup(Alignment.LEADING)
					.addGroup(gl_panelContent.createSequentialGroup()
						...
						.addGroup(gl_panelContent.createParallelGroup(Alignment.LEADING)
							...
							.addGroup(gl_panelContent.createSequentialGroup()
								.addComponent(btnDane, GroupLayout.PREFERRED_SIZE, 104, GroupLayout.PREFERRED_SIZE)
							...
							.addGroup(gl_panelContent.createSequentialGroup()
								.addComponent(btnAdres, GroupLayout.PREFERRED_SIZE, 107, GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(ComponentPlacement.RELATED)
							...
							.addGroup(gl_panelContent.createSequentialGroup()
								.addComponent(btnDaty, GroupLayout.PREFERRED_SIZE, 109, GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(ComponentPlacement.RELATED)
		...
		}
		{
			...
			}
			{
				...
				btnCansel.setIcon(new ImageIcon(EditParishionerDialog.class
	...
	public Parishioner getParishionerEdited() {
		...
		if (parishioner.getAdress() == null)
			parishioner.setAdress(new Adress());
		...
		if (parishioner.getCourse() == null)
			parishioner.setCourse(new Course());
		

		parishioner.getCourse().setBirthday(
				(!dateBirthday.isEmpty()) ? dateBirthday.getDate() : null);

		parishioner.getCourse().setBaptism(
				(!dateBaptism.isEmpty()) ? dateBaptism.getDate() : null);

		parishioner.getCourse().setCommunion(
				(!dateCommunion.isEmpty()) ? dateCommunion.getDate() : null);

		parishioner.getCourse().setConfirmation(
				(!dateConfirmation.isEmpty()) ? dateConfirmation.getDate()
						: null);

		parishioner.getCourse().setMarriage(
				(!dateMarriage.isEmpty()) ? dateMarriage.getDate() : null);

		parishioner.getCourse().setDeath(
				(!dateDeath.isEmpty()) ? dateDeath.getDate() : null);
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (z == btnCansel) {
			...
			if (this.getParishionerEdited().getData().equals("ERR"))
				return;
			isDane = true;
			setVisible(false);
		} else if (z == btnAdres) {
